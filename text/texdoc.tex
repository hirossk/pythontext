...
今回はコンピューターをなにかしらの目的を持ってどうさせるための手順という意味でプログラムを作ること（プログラミング）について学んでいきます。
...
\begin{description}
	\item $https://www.python.jp/install/windows/install_py3.html$
\end{description}
...
いままでも対話型の命令実行では結果の出力はできていましたが、明示的に出力する方法を学びます。
...
保存が完了しましたら、対話型実行で利用したターミナルに切り替えます。

\begin{hipoint}{Point}
対話型で動作している場合はターミナルで「$>>>　$」の後に、CTRL+Zを押すか「exit()」を実行することでコマンド実行状態に変更できます。
\end{hipoint}
...
※ 対話型で変数の値を出力するために変数名だけでよかった理由は式の評価した値が表示されたためです。
...
対話型の実行で表示されている「$>>>$」の後に入力する命令や式は実行され「評価」されます。
\end{hipoint}
...
このように対話型の実行では条件式や計算式の評価結果を表示しますので、print命令による出力と違いがわかりません。
...
ファイル名「python2-1.py」として保存してください。
\end{plabox}
...
ファイル名python2-2.pyとして保存してください。（ファイル名は変更しません）
\end{plabox}

\section{キーボードからの入力}
プログラムを実行時に変数に値をキーボードから入力した値に実行後設定してみましょう。このような値の代入を動的な値の代入といいます。

対話型で命令を実行してキーボードからの入力をしてみましょう。
...
ファイル名：python3-1.pyとして保存し実行しましょう。

※　入力が数値として判定できない場合の処理は行いません。
\begin{legbox}{python3-1.py}
\begin{listing}{1}
x = input ( “math = “ )
m = int ( x ) 
y = input ( “english = “ )
...
ファイル名はpython3-1.pyとして保存してください。（ファイル名は変更しません）
...
ファイル名はpython4-1.pyとして保存してください。（ファイル名は変更しません）
\end{plabox}
...
ファイル名をpython4-3.pyとして保存してください。
\begin{legbox}{python4-3.py}
\begin{listing}{1}
sum = 0
for x in range(10):
    sum = sum + x
...
ファイル名：python4-3.py（ファイル名は変更しません）
\end{plabox}
...
ファイル名はpython4-4.pyとして保存してください。（ファイル名は変更しません）
\end{plabox}
...
ファイル名：python5-1.py（ファイル名は変更しないでください）
\begin{legbox}{python5-1.py}
\begin{listing}{1}
subjects = \
...
ファイル名python5-1.pyとして保存しましょう。（ファイル名は変更しないでください。）
\begin{legbox}{python5-1.py}
\begin{listing}{1}
subjects = \
        [“Mathematics”,”English”,”Social”,”Japanese”,”Scientific”]
...
\subsubsection{練習問題}
\begin{plabox}{Plactice}
ここまでの学習内容を復習して5科目の点数を入力し、0点未満や100点を超える点数は「エラー」と表示し入力された場合は得点に-1を設定するようにしましょう。
...
\subsubsection{練習問題}
\begin{plabox}{Plactice}
「Plactice7」のプログラムを３人分の得点を入力して5科目それぞれの平均値を出力するように変更してください。
...
\end{pabox}
対話実行で定義の無い関数を呼び出すと次のエラーが出力されます。
...
ファイル名python7-1.pyで保存してください。
\begin{codebox}{グローバル変数を利用した関数}
\begin{listing}{1}
#引数にはなにも指定しません。
def func():
    answer = x ** 2 + 2 * x + 3
...
\begin{codebox}{ローカル変数とグローバル変数を利用した関数}
\begin{listing}{1}
...
※　Plactice7を関数evaluationを利用して記載することができます。
\begin{codebox}{関数を利用した処理}
\begin{listing}{1}
...
\begin{plabox}{Plactice}
次の実行例のプログラム、3名の学生の5科目の得点を入力してそれぞれの平均値を出力するように変更してください。
この時「error」な点数の入力があれば-1点として平均値に参入しないものとします。
...
プログラミングは今まで見てきた手続きをまとめながら記述する方法で記載されます。

プログラムの規模が大きくなるともっと効率よく書く方法が考えられるようになりました。

これがオブジェクト指向プログラミングといいます。オブジェクト指向プログラミングではプログラムコードの再利用を優先して考えていくことを優先しています。

この中で重要なキーワードがクラスといわれるプログラムで扱うデータと処理（メソッド）を一体化した仕組みです。
...
\begin{ansbox}{Plactice1（python2-1.py）}
\begin{listing}{1}
x = 3
y = 4
print ( (x * y) /2)
...
\begin{ansbox}{Plactice2（python2-1.py）}
\begin{listing}{1}
x = 60
if x >= 50:
    print ( " goukaku " ) 
...
\begin{ansbox}{Plactice3（python3-1.py）}
\begin{listing}{1}
x = 1 
y = 2
...
\begin{ansbox}{Plactice4（python2-2.py）}
\begin{listing}{1}
x = 60
if 100 >= x >= 80:
    print ( “A” )
...
\begin{ansbox}{Plactice5（python3-1.py）}
\begin{listing}{1}
x = input ( “math = “ )
m = int ( x ) 
e = input ( “english = “ )
...
\begin{ansbox}{Plactice6（python4-1.py）}
\begin{listing}{1}
for x in range(1000):
    print( “wan” )
    print( “nyan” )
...
\begin{ansbox}{Plactice7（python4-2.py）}
\begin{listing}{1}
sum = 0
...
\begin{ansbox}{Plactice8（python4-3.py）}
\begin{listing}{1}
y = 0
for n in range(1,6):
...
\begin{ansbox}{Plactice9（python5-2.py）}
\begin{listing}{1}
subjects = ["Mathematics" ,"English","Social","Japanese","Scientific"]
scores = []
for s in subjects:
...
\begin{ansbox}{Plactice10（python5-3.py）}
\begin{listing}{1}
students_scores = []
#配列の準備
students_scores.append([60, 80]) #Aさんのデータを追加
...
\begin{ansbox}{Plactice11（python5-3.py）}
\begin{listing}{1}
subjects = ["Mathematics" ,"English","Social","Japanese","Scientific"]
scores = []
for n in range(3):
...
\begin{ansbox}{Plactice12（python5-3.py）}
\begin{listing}{1}
def func1(n):
    a = (n - 65)**2
    return a
...
\begin{ansbox}{Plactice13（python5-3.py）}
\begin{listing}{1}
zzz
\end{listing}
\end{ansbox}
