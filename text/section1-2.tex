...
今回利用するpython言語はご案内の通りGoogle Colaboratoryを利用します。


\includegraphics[width=10cm]{images/colab01.png}

Webで起動すると次の画面が表示されます。

\includegraphics[width=12cm]{images/colab02.png}
...
\includegraphics[width=8cm]{images/colab04.png}
...
いままでも {\gt コードセルによる実行}では結果の出力はできていましたが、明示的に出力する方法を学びます。


\subsection{画面に出力する方法}
結果を出力するprint文を使ってみます。
\begin{grabox}{出力命令}
\includegraphics[width=7cm]{images/colab10.png}
\end{grabox}
「print(x)」も「x」もxの値が出力されます。違いについては次のセクション「コードセルでの出力」の中で説明します。
...
\includegraphics[width=7.5cm]{images/colab11.png}
\end{grabox}
実行すると「print( x )」の値だけが出力されることが分かります。

明示的に変数の内容を確認するにはprint関数を利用します。
\begin{grabox}{プログラム名（python1-2.py）}
\begin{listing}{1}
x = 1
x = x + 2
print (x)
...
※ 今まで実行結果として変数名だけで内容が表示された理由は式の評価した値が結果として表示されたためです。
...
\includegraphics[width=7.5cm]{images/colab13.png}
\end{grabox}
...
このように {\gt コードセルによる実行}では最後の評価結果を表示しますので、コードセルの途中で出力したい場合はprint命令によって明示的に出力する必要があります。
...
\includegraphics[width=12cm]{images/colab16.png}
\end{legbox}
...
\includegraphics[width=12cm]{images/colab20.png}
\end{grabox}
...
\includegraphics[width=7cm]{images/colab24.png}
...
\includegraphics[width=10cm]{images/colab25.png}
\end{grabox}
...
\includegraphics[width=13cm]{images/colab26.png}
...
\includegraphics[width=13cm]{images/colab27.png}
